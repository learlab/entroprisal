...
def test_entropy_reduction_invalid_n(rich_ngrams):
    """Entropy reduction supports only n in {2, 3}; others (incl. n=1) are rejected."""
    calc = TokenEntropisalCalculator(rich_ngrams, min_frequency=10)
    with pytest.raises(ValueError):
...
def test_entropy_difference_lower_order(rich_ngrams):
    """n=2 (trigram) entropy difference: E_2[t-1] - E_2[t] over 2-token contexts."""
    calc = TokenEntropisalCalculator(rich_ngrams, min_frequency=10)
    df = calc.entropy_difference(["the", "cat", "sat", "down", "hard"], n=2)

    # E_2[down] context (cat,sat) = H({down:100, up:100}) = 1.0 bit;
    # E_2[hard] context (sat,down) = H({hard:70}) = 0 -> diff at t=4 is 1.0.
    row = df[df["position"] == 4].iloc[0]
    assert row["entropy_difference"] == pytest.approx(1.0)
    assert bool(row["available"]) is True


def test_entropy_difference_invalid_n(rich_ngrams):
    """Entropy difference supports only n in {2, 3}; others (incl. n=1) are rejected."""
    calc = TokenEntropisalCalculator(rich_ngrams, min_frequency=10)
    with pytest.raises(ValueError):
        calc.entropy_difference(["the", "cat", "sat", "down"], n=1)
    with pytest.raises(ValueError):
...
def test_calculate_metrics_includes_entropy_keys(rich_ngrams):
    ...
    assert "entropy_difference_2_support" in metrics
